...
| HW-1.04 | **VCO_in DC transfer check** | Apply a constant **1 V DC** level to the **VCO_in** path | Output at VCO_in pin = **1.091 V DC ± 1.653 mV** and stable | 1.0418 V | <span style="color:yellow;">**failed, acceptable**</span> | Out of tolerance, can be compsensated in code |
...
- Out-of-band attenuation checks pass if relative attenuation at 0.1 Hz and 40 Hz is -17 dB ± 20% versus the midband response
...
| ID | Test description | Input signals / setup | Expected result | Measured result | Verdict | Remarks |
| - | - | - | - | - | - | - |
| HW-2.01 | **DC operating point** | Power the PCB under nominal operating conditions with both I and Q inputs grounded | Outputs biased at **1.5 V** DC on both paths | 1.5 V ± 100 mV | <span style="color:red;">**failed**</span> | Should be compensated in code |
...
| HW-2.05 | **I-path cut-off check** | Apply sine to **I input only**. Ground **Q input**. Frequencies: **0.3 Hz** and **13 Hz**. Run once at **50 µVpp** and once at **250 µVpp** | Attenuation **-3 dB ± 20%** relative to midband response at both cut-off points. Expected output ≈ **355 mVpp** and **1.78 Vpp** respectively, centered around **1.5 V DC** | ... | ... | ... |
...
| HW-2.08 | **I-path out-of-band attenuation check** | Apply sine to **I input only**. Ground **Q input**. Frequencies: **0.1 Hz** and **40 Hz**. Run once at **50 µVpp** and once at **250 µVpp** | Attenuation **-17 dB ± 20%** relative to midband response at both out-of-band points. Expected output ≈ **71 mVpp** and **355 mVpp** respectively, centered around **1.5 V DC** | ... | ... | ... |
