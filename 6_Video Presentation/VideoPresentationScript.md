...
| 11 | The prototype is intended for short-range operation. In the user interface, the displayed radar spectrum covers approximately plus or minus four hertz. ECG reference measurement and SD-card logging support validation and later analysis. | Static technical summary slide: `Short-range prototype`, `Spectrum: +/-4 Hz`, `ECG reference`, `Radar summary logging`. If measured, add: `Tested range: ...`. | 2:33-2:45 |
| 12 | The radar FFT bin spacing is approximately 0.195 hertz, or 11.7 beats per minute. The radar BPM estimator refines candidate peaks with parabolic interpolation, so the output is not limited to raw FFT-bin steps. Practical accuracy still depends on signal quality, leakage, body motion, and temporal stability. | Technical graphic: FFT peak between bins, parabolic curve over three bins. Labels: `0.195 Hz`, `11.7 bpm/bin`, `sub-bin interpolation`, `accuracy depends on signal quality` | 2:46-3:06 |
| 13 | Contactless radar sensing, real-time embedded processing, touchscreen visualization, ECG-supported validation, and SD-card radar-summary logging come together in one compact platform for comfortable vital-sign monitoring. | **Final slide:** project title, authors, date, ZHAW affiliation, repository/project link or QR code. | 3:07-3:17 |
