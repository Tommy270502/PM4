...
- `subgraph ... end` groups one subsystem.
- `A --> B` means flow from one step to the next.
- Text inside `["..."]` is a process box.
- `<br/>` makes a new line inside a box.

Based on the current implementation in `main.c`, `radar.c`, `ekg.c`, `display.c`, `menu.c`, `pushbutton.c`, `fft.c`, `filters.c`, and `dac_output.c`.
