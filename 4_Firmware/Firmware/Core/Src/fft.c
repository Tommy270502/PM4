...
HAL_StatusTypeDef fft_iq_centered(const float32_t i_in[], const float32_t q_in[],
                                  float32_t out[])
{
    static float32_t complex_buffer[2U * RADAR_CHANNEL_SAMPLES];
    static float32_t magnitude_buffer[RADAR_CHANNEL_SAMPLES];
    ...
    for (uint32_t i = 0; i < RADAR_CHANNEL_SAMPLES; i++)
    {
        ...
        complex_buffer[2U * i] = windowed_i;
        complex_buffer[(2U * i) + 1U] = windowed_q;
    }

    arm_cfft_f32(fft_instance, complex_buffer, 0U, 1U);
    arm_cmplx_mag_f32(complex_buffer, magnitude_buffer, RADAR_CHANNEL_SAMPLES);
    arm_scale_f32(magnitude_buffer, fft_abs_scale, magnitude_buffer,
                  RADAR_CHANNEL_SAMPLES);

    for (uint32_t i = 0; i < half_size; i++)
    {
        out[i] = magnitude_buffer[i + half_size];
        out[i + half_size] = magnitude_buffer[i];
    }
